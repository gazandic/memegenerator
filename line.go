...
type PythonTessResponse struct {
...
func (app *KitchenSink) handleImage(message *linebot.ImageMessage, replyToken string) error {
	return app.handleHeavyContent(message.ID, func(originalContent *os.File) error {
		...
		resp, err := http.Get("http://127.0.0.1:5000/telephone/"+originalContent.Name())
		if err != nil {
			return err
		}
    ...
    fmt.Println("body: <%s>", string(body))
		...
		s, err := getTessResponse([]byte(body))
		if err != nil {
		    panic(err.Error())
		}
		...
		if s.Result == "" {
			if _, err := app.bot.ReplyMessage(
				replyToken,
				linebot.NewTextMessage("sekar chan tidak lihat ada tulisan hehe"),
			).Do(); err != nil {
				return err
			}
		}
		if s.Result != "" {
			if _, err := app.bot.ReplyMessage(
				replyToken,
				linebot.NewTextMessage(s.Result),
			).Do(); err != nil {
				return err
			}
		}
...
func getTessResponse(body []byte) (*PythonTessResponse, error) {

		var s = new(PythonTessResponse)
